import { ChakraProvider, Box, Button, Heading, Flex, Select, Textarea, FormControl, FormErrorMessage } from '@chakra-ui/react';
    ...
    <ChakraProvider>
      ...
      <Box minHeight="100vh" bg="#f0f4f8" color="#34495e">
        ...
        <Box py={10} px={6} maxW="1000px" mx="auto">
          ...
          <Flex direction={{ base: 'column', md: 'row' }} justifyContent="space-between" mb={6}>
            <Box width={{ base: '100%', md: '48%' }} mb={{ base: 4, md: 0 }}>
              <Heading size="md" mb={4}>Explorer Setup</Heading>
              <FormControl isInvalid={errors.explorerAgent}>
                ...
                {errors.explorerAgent && <FormErrorMessage>{errors.explorerAgent}</FormErrorMessage>}
              </FormControl>
              ...
              <FormControl isInvalid={errors.explorerDescription} mt={4}>
                <Textarea
                  ...
                  color="#34495e"
                  border="2px solid"
                  borderColor={errors.explorerDescription ? 'red.500' : '#ecf0f1'}
                  _hover={{ borderColor: '#3498db' }}
                  p={3}
                  minHeight="100px"
                />
                {errors.explorerDescription && <FormErrorMessage>{errors.explorerDescription}</FormErrorMessage>}
              </FormControl>
            </Box>

            <Box width={{ base: '100%', md: '48%' }}>
              <Heading size="md" mb={4}>Terminal Setup</Heading>
              <FormControl>
                <Select
                  value={terminalAgent}
