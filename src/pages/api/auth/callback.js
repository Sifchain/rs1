  ...
  try {
    ...
    if (!codeVerifier || state !== agent.twitterAuthState.state) {
      console.error('State mismatch:', {
        storedState: agent.twitterAuthState.state,
        state,
      })
    ...
    if (!accessToken || !refreshToken) {
      console.error('Twitter API did not return tokens:', {
        accessToken,
        refreshToken,
      })
      return res
    ...
    res.redirect(
      `/create-agent?agent=${encodeURIComponent(agent._id)}&twitterLinked=true`
    )
  } catch (error) {
