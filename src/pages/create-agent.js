...
`;
  ...
  const { hasCopied: convoCopied, onCopy: copyConversation } = useClipboard(conversationPromptTemplate);
  const { hasCopied: recapCopied, onCopy: copyRecap } = useClipboard(recapPromptTemplate);
  const { hasCopied: tweetCopied, onCopy: copyTweet } = useClipboard(tweetPromptTemplate);
  ...
  return (
    <ChakraProvider>
      <SEO title="Create an Agent" description="Create and link an agent with Twitter" />
            ...
            <Flex direction="column" gap={6}>
              <VStack spacing={4} align="stretch">
                <FormControl isInvalid={errors.agentName}>
                  <Text fontWeight="bold" color="#2980b9">Agent Name</Text>
                  <Input
                ...
                <FormControl isInvalid={errors.traits}>
                  <Text fontWeight="bold" color="#2980b9">Traits</Text>
                  <Textarea
                    ...
                    rows={4}
                  />
                  {errors.traits && <FormErrorMessage>{errors.traits}</FormErrorMessage>}
                </FormControl>

                {/* Description Section */}
                <Box border="1px solid #ecf0f1" borderRadius="md" p={4}>
                  <Flex justify="space-between" align="center">
                    <Text fontWeight="bold" color="#2980b9">Description</Text>
                    <Button variant="link" colorScheme="blue" size="sm" onClick={onToggle}>
                      Template Guide {isOpen ? '▲' : '▼'}
                    </Button>
                  </Flex>
                  <Collapse in={isOpen} animateOpacity>
                    <Box mt={4} p={4} bg="#f7fafc" borderRadius="md" fontSize="sm">
                ...
                {[{ title: 'Backroom Prompt', template: conversationPromptTemplate, value: conversationPrompt, setter: setConversationPrompt, open: convoOpen, toggle: toggleConversation, copy: copyConversation, copied: convoCopied },
                  { title: 'Training Prompt', template: recapPromptTemplate, value: recapPrompt, setter: setRecapPrompt, open: recapOpen, toggle: toggleRecap, copy: copyRecap, copied: recapCopied },
                  { title: 'Recap Prompt', template: tweetPromptTemplate, value: tweetPrompt, setter: setTweetPrompt, open: tweetOpen, toggle: toggleTweet, copy: copyTweet, copied: tweetCopied }].map((prompt, index) => (
                    <Box key={index} border="1px solid #ecf0f1" borderRadius="md" p={4} mt={4}>
                      <Flex justify="space-between" align="center">
                        <Text fontWeight="bold" color="#2980b9">{prompt.title} (Optional)</Text>
                        <Button variant="link" colorScheme="blue" size="sm" onClick={prompt.toggle}>
                ...
                <Button onClick={handleSubmit} colorScheme="blue" width="100%" mt={4}>
          ...
          {loadingStep > 0 && loadingStep < 3 && (
            <Box textAlign="center" mt={6}>
              ...
              <Heading fontSize="lg" mt={4}>
                {loadingStep === 1 ? 'Contacting Spiral Reality AI...' : 'Validating Agent Details...'}
              </Heading>
            </Box>
          )}
            ...
            <Box textAlign="center" mt={6}>
              <Alert status="success" mb={6}>
                ...
                <Box>
                  ...
                  <AlertDescription>Your agent has been created and is ready for further actions.</AlertDescription>
                </Box>
              </Alert>
              {twitterLinked && (
                <Alert status="info" mb={6}>
                  ...
                  <Box>
                    ...
                    <AlertDescription>Your Twitter account has been successfully linked.</AlertDescription>
                  </Box>
                </Alert>
              )}
              <Button onClick={handleTwitterAuth} colorScheme="twitter" width="100%" disabled={twitterLinked}>
                {twitterLinked ? 'Twitter Account Linked' : 'Link Twitter Account'}
              </Button>
              <Button onClick={handleCreateBackroom} colorScheme="green" width="100%" mt={4}>
                Create Backroom
              </Button>
            </Box>
          )}
